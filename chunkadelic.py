...
def load_file(filename, sr=4800):
...
def blow_chunks(audio, new_filename, chunk_size, sr=48000):
    "chunks up the audio and saves them with --{i} on the end of each chunk filename"
    n_chunks = math.ceil( audio.shape[-1] / chunk_size )
...
def process_one_file(filenames, chunk_size, sr, output_path, input_paths, file_ind):
    ...
    blow_chunks(audio, new_filename, chunk_size, sr)
    return
...
def main():
    ...
    wrapper = partial(process_one_file, filenames, args.chunk_size, args.sr, args.output_path, args.input_paths)
