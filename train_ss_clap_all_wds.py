...
class LatentDiffusionCond(pl.LightningModule):
    def __init__(self, autoencoder: AudioAutoencoder):
        ...
        self.embedder = laion_clap.CLAP_Module(enable_fusion=False).requires_grad_(False).eval()

        self.embedder.load_ckpt(model_id=1)
       ...
       # self.timestamp_embedder = NumberEmbedder(features=self.embedding_features)
        ...
        self.diffusion = UNetCFG1d(
            ...
            attention_rel_pos_max_distance=1024,
...
def main():
    ...
    autoencoder.load_state_dict(torch.load(args.pretrained_ckpt_path, map_location=device)["state_dict"])
    ...
    if args.ckpt_path:
        latent_diffusion_model = LatentDiffusionCond.load_from_checkpoint(args.ckpt_path, latent_ae=autoencoder, strict=False)
    else:
        latent_diffusion_model = LatentDiffusionCond(autoencoder=autoencoder)
