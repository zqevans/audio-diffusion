...
from blocks.embedders import TextEmbedder
...
class LatentAudioDiffusionAutoencoder(pl.LightningModule):
    def __init__(self, autoencoder: AudioAutoencoder):
        ...
        self.downsampling_ratio = autoencoder.downsampling_ratio * \
            self.latent_downsampling_ratio
    ...
    def decode(self, latents, steps=250, device="cuda"):
        first_stage_latent_noise = torch.randn(
            [latents.shape[0], self.latent_dim, latents.shape[2]*self.latent_downsampling_ratio]).to(device)

        first_stage_sampled = sample(
            self.diffusion, first_stage_latent_noise, steps, 0, cond=latents)
...
class StackedAELatentDiffusion(pl.LightningModule):
    def __init__(self, latent_ae: LatentAudioDiffusionAutoencoder, sample_size=524288):
        super().__init__()
        ...
        self.embedder = TextEmbedder(
            model='t5-base', max_length=embedding_max_len).requires_grad_(False)

        self.embedding_features = 768

        self.diffusion = DiffusionTransformer(
            io_channels=self.latent_dim,
            input_length=sample_size // self.downsampling_ratio,
            embed_dim=1024,
            depth=16,
            num_heads=16,
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                ...
                text_embeddings, embeddings_mask = self.embedder(
                    condition_strings)
        ...
        with torch.cuda.amp.autocast():
            # 0.1 CFG dropout
            v = self.diffusion(noised_latents, t, cond_tokens=embeddings,
                               cond_token_mask=embeddings_mask, cfg_dropout_prob=0.1)
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            ...
            latent_noise = torch.randn(
                [8, module.latent_dim, self.demo_samples//module.downsampling_ratio]).to(module.device)

            with torch.cuda.amp.autocast():
                text_embeddings, text_embeddings_masks = module.embedder([
                    "",
                    "",
                    "",
                    "",
            ...
            for cfg_scale in demo_cfg_scales:
                ...
                with torch.cuda.amp.autocast():
                    fake_latents = sample(module.diffusion_ema, latent_noise, self.demo_steps, 0,
                                          cond_tokens=embeddings, cond_token_mask=text_embeddings_masks, cfg_scale=cfg_scale)
                ...
                log_dict[f'demo_melspec_left_{cfg_scale}'] = wandb.Image(
                    audio_spectrogram_image(fakes))

                log_dict[f'embeddings_3dpca_{cfg_scale}'] = pca_point_cloud(
                    fake_latents)
                log_dict[f'embeddings_spec_{cfg_scale}'] = wandb.Image(
                    tokens_spectrogram_image(fake_latents))
...
def main():
    ...
    first_stage_config = {"capacity": 64, "c_mults": [
        2, 4, 8, 16, 32], "strides": [2, 2, 2, 2, 2], "latent_dim": 32}
    ...
    latent_diffae = LatentAudioDiffusionAutoencoder.load_from_checkpoint(
        args.pretrained_ckpt_path, autoencoder=first_stage_autoencoder, strict=False)
    ...
    diffusion_trainer.fit(latent_diffusion_model,
                          train_dl, ckpt_path=args.ckpt_path)
