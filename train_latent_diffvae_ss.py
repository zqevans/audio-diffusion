...
class LatentAudioDiffusionAutoencoder(pl.LightningModule):
    def __init__(self, global_args, autoencoder: AudioAutoencoder):
        ...
        # dimension of the conditioning signal for the diffusion model
        self.cond_dim = self.second_stage_latent_dim

        self.latent_encoder = Encoder1d(
            in_channels=self.latent_dim, 
            out_channels = self.second_stage_latent_dim * 2,
            channels = 128,
        ...
        self.latent_decoder = Decoder1d(
            in_channels = self.second_stage_latent_dim, 
            out_channels = self.cond_dim,
            channels = 128,
            multipliers = [8, 8, 4, 2, 1],
            factors =  [2, 2, 2, 2],
            num_blocks = [4, 4, 4, 4],
        )

        self.latent_decoder_ema = deepcopy(self.latent_decoder)
        ...
        self.latent_decoder_ema.requires_grad_(False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            ...
            second_stage_decoded = self.latent_decoder(second_stage_latents)

            v = self.diffusion(noised_latents, t, second_stage_decoded)
...
class DemoCallback(pl.Callback):
    def __init__(self, demo_dl, global_args):
        ...
        self.num_demos = global_args.batch_size
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):   
        ...
        try:
            ...
            with torch.no_grad():
                ...
                mean, scale = module.latent_encoder_ema(first_stage_latents).chunk(2, dim=1)
                second_stage_latents, _ = module.sample(mean, scale)
                second_stage_decoded = module.latent_decoder_ema(second_stage_latents)
            ...
            recon_latents = sample(module.diffusion_ema, latent_noise, self.demo_steps, 0, second_stage_decoded)
