...
from a_unet import TextConditioningPlugin, TimeConditioningPlugin, ClassifierFreeGuidancePlugin, T5Embedder, NumberEmbedder
...
class StackedAELatentDiffusionCond(pl.LightningModule):
    def __init__(self, latent_ae: LatentAudioDiffusionAutoencoder):
        ...
        resnet_blocks = [1, 1, 1, 1, 1, 1, 1, 1, 1]

        attentions = [0, 0, 0, 1, 1, 1, 1, 1, 1]

        cross_attentions = [0, 1, 1, 0, 0, 0, 0, 0, 0]

        items = [3, 3, 3, 3, 3, 3, 3, 3, 3]
        
        block_defs = channels, factors, resnet_blocks, attentions, cross_attentions, items

        # Add time conditioning and CFG
        UNet = ClassifierFreeGuidancePlugin(XUNet, embedding_max_length=embedding_max_len + 2) #2 for timestamp embeds
        ...
        self.diffusion = UNet(
            ...
            blocks = [
                XBlock(
                    ...
                    factor = factor,
                    items=([ResnetItem, ModulationItem] * num_resnet_blocks + [AttentionItem, CrossAttentionItem] * num_attns + [CrossAttentionItem] * num_cross_attentions + [ResnetItem, ModulationItem]) * num_items
                ) for num_channels, factor, num_resnet_blocks, num_attns, num_cross_attentions, num_items in zip(*block_defs)
            ],
            attention_heads = 16,
            attention_features = 64,
            attention_multiplier = 4,
            skip_t = SkipCat,
            resnet_kernel_size = 5,
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            # 0.1 CFG dropout
            v = self.diffusion(noised_latents, time=t, embedding=embeddings, embedding_mask_proba = 0.1)
...
def main():
    ...
    train_dl = get_wds_loader(batch_size=args.batch_size, s3_url_prefix="s3://s-harmonai/datasets/", sample_size=args.sample_size, names=names, sample_rate=args.sample_rate, num_workers=args.num_workers, recursive=True)
