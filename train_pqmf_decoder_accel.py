...
class PQMFDecoder(nn.Module):
    ...
    def loss(self, reals):
        ...
        with torch.cuda.amp.autocast():
            v = self.diffusion(noised_reals, t, encoder_input)
...
def main():
    ...
    args.latent_dim = 2*args.pqmf_bands
    ...
    diffusion_model = PQMFDecoder(args, device)
    ...
    def demo():
        ...
        fakes = sample(model_ema.diffusion, noise, args.demo_steps, 1, encoder_input)
        ...
        if use_wandb:
            try:
                ...
                log_dict[f'embeddings_spec'] = wandb.Image(tokens_spectrogram_image(encoder_input))
