...
from a_transformers import Resampler
...
class StackedAELatentDiffusionCond(pl.LightningModule):
    def __init__(self, latent_ae: LatentAudioDiffusionAutoencoder, sample_size = 1572864):
        super().__init__()
        ...
        audio_embedding_len = sample_size // self.downsampling_ratio

        self.to_audio_embeddings = Encoder1d(
            in_channels = self.latent_dim,
        ...
        self.to_audio_embeddings_ema = EMA(
            self.to_audio_embeddings,
            beta = 0.9999,
            power=3/4,
            update_every = 1,
        ...
        self.resampler = Resampler(
            ...
            in_tokens=embedding_max_len + audio_embedding_len,
            out_tokens=embedding_max_len, 
            num_layers=8,
            head_features=64,
            num_heads=12,
            multiplier=4,
        ...
        self.diffusion = UNetConditional1d(
            ...
            num_blocks = [3, 3, 3, 3, 3, 3, 4],
            attentions = [0, 0, 0, 3, 3, 3, 3, 3],
            attention_heads = 16,
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam([*self.diffusion.parameters(), *self.resampler.parameters(), *self.to_audio_embeddings.parameters()], lr=1e-4)
    ...
    def on_before_zero_grad(self, *args, **kwargs):
        self.to_audio_embeddings_ema.update()
        self.resampler_ema.update()
