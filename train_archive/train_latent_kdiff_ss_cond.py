...
def sample(model, x, steps, eta, **extra_args):
    ...
    for i in trange(steps):
        ...
        with torch.cuda.amp.autocast():
            #mapping_cond=mapping_cond, unet_cond=unet_cond, log_sigma=False
...
class LatentAudioDiffusionAutoencoder(pl.LightningModule):
    def __init__(self, global_args, autoencoder: AudioAutoencoder, embedder: T5Embedder = None):
        ...
        self.diffusion = AudioDenoiserModel(
            ...
            feats_in = self.embedding_features,
            depths = [3, 3, 3, 3, 3, 3, 4, 4, 4, 4, 4],
            channels = [512] * 11,
            self_attn_depths = [0]*11,
            strides = [2]*10,
            dropout_rate=0.0,
            unet_cond_dim=second_stage_latent_dim,
            mapping_cond_dim=self.embedding_features,
        )
    ...
    def configure_optimizers(self):
        return optim.Adam([*self.latent_encoder.parameters(), *self.resampler.parameters(), *self.diffusion.parameters()], lr=1e-4)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.cuda.amp.autocast():
            ...
            v = self.diffusion(noised_latents, t, 
                    unet_cond=second_stage_latents,
                    mapping_cond=features,
                    log_sigma=False)
...
class DemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):   
        ...
        try:
            ...
            demo_reals, demo_filenames = next(self.demo_dl)
            ...
            with torch.no_grad():
                ...
                features = module.resampler_ema(embedding).squeeze()
