...
@torch.no_grad()
def sample(model, x, steps, eta, logits, guidance_scale=1.):
    ...
    for i in trange(steps):
        ...
        with torch.cuda.amp.autocast():
            ...
            cond = torch.cat([logits, torch.zeros_like(logits)])
            v_cond, v_uncond = model(x_in, ts_in * t[i], cond).float().chunk(2)
...
class DiffusionDVAE(pl.LightningModule):
    def __init__(self, global_args):
        ...
        self.diffusion = DiffusionAttnUnet1D(global_args, n_attn_layers=0, c_mults=[256] * 3 + [512] * 6 + [1024]*5)
    ...
    def training_step(self, batch, batch_idx):
        ...
        p = torch.rand([tokens.shape[0], 1, 1], device=tokens.device)
...
class DemoCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, module):
        ...
        with torch.no_grad():
            ...
            uncond_fakes = sample(module.diffusion_ema, noise, self.demo_steps, 1, torch.randn_like(tokens))

        # Put the demos together
        uncond_fakes = rearrange(uncond_fakes, 'b d n -> d (b n)')
        ...
        try:
            ...
            uncond_filename = f'uncond_{trainer.global_step:08}.wav'
            uncond_fakes = uncond_fakes.clamp(-1, 1).mul(32767).to(torch.int16).cpu()
            torchaudio.save(uncond_filename, uncond_fakes, self.sample_rate)
            ...
            log_dict[f'real'] = wandb.Audio(reals_filename,
                                                sample_rate=self.sample_rate,
                                                caption=f'Real')
            log_dict[f'uncond'] = wandb.Audio(uncond_filename,
                                                sample_rate=self.sample_rate,
                                                caption=f'Unconditional')
            ...
            log_dict[f'diff_melspec_left'] = wandb.Image(audio_spectrogram_image(demo_reals - fakes))
