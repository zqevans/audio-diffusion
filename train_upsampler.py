...
@torch.no_grad()
def sample(model, x, steps, eta, cond):
...
class DiffusionUpsampler(pl.LightningModule):
    def __init__(self, global_args, sample_rate=48000, upsample_ratio=2):
        super().__init__()
        ...
        self.downsampler = T.Resample(sample_rate, sample_rate // upsample_ratio)
    ...
    def training_step(self, batch, batch_idx):
        ...
        downsampled = self.downsampler(reals)

        with torch.cuda.amp.autocast():
            v = self.diffusion(noised_reals, t, downsampled)
...
class DemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):   
        ...
        if self.pqmf_bands > 1:
            demo_reals = self.pqmf(demo_reals)

        demo_reals = demo_reals.to(module.device)

        downsampled = module.downsampler(demo_reals)

        noise = torch.randn([demo_reals.shape[0], 2, self.demo_samples]).to(module.device)

        upsampled = sample(module.diffusion_ema, noise, self.demo_steps, 0, downsampled)

        # Put the demos together
        upsampled = rearrange(upsampled, 'b d n -> d (b n)')
        downsampled = rearrange(downsampled, 'b d n -> d (b n)')
        ...
        try:
            ...
            torchaudio.save(downsample_filename, downsampled, self.sample_rate // module.upsample_ratio)
            ...
            log_dict[f'downsampled'] = wandb.Audio(downsample_filename,
                                                sample_rate=self.sample_rate // module.upsample_ratio,
...
def main():
    ...
    diffusion_model = DiffusionUpsampler(args, args.sample_rate, args.upsample_ratio)
