...
class StackedAELatentDiffusionCond(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam([*self.diffusion.parameters(),  
        *self.second_start_embedder.parameters(), *self.second_total_embedder.parameters() ], lr=4e-5)
    ...
    def training_step(self, batch, batch_idx):
        ...
        seconds_starts_totals = [[json["seconds_start"][0], json["seconds_total"][0]] for json in jsons]
        
        #print(f'seconds_starts_totals: {seconds_starts_totals}')
        #print(condition_strings)

        with torch.cuda.amp.autocast():
            timing_embeddings = self.get_timing_embeddings(seconds_starts_totals)
            #print(f"timing_embeddings: {timing_embeddings}")
            with torch.no_grad():
                first_stage_latents = self.autoencoder.autoencoder.encode(reals)

                second_stage_latents = self.autoencoder.latent_encoder(first_stage_latents)
        ...
        noise = torch.randn_like(second_stage_latents)
        noised_latents = second_stage_latents * alphas + noise * sigmas
        targets = noise * alphas - second_stage_latents * sigmas
        ...
        log_dict = {
            ...
            'train/latent_std': second_stage_latents.std(),
            'train/first_stage_latent_std': first_stage_latents.std(),
        }
...
class DemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        try:
            latent_noise = torch.randn([2, module.latent_dim, self.demo_samples//module.downsampling_ratio]).to(module.device)
            ...
            timing_embeddings = module.get_timing_embeddings(
                [
