...
import torch
import logging
from diffusion.FastDiff.modules import DiffusionDBlock, TimeAware_LVCBlock
from diffusion.FastDiff.util import calc_diffusion_step_embedding
...
class FastDiff(nn.Module):
    ...
    def __init__(self,
        ...
        self.first_audio_conv = nn.Conv1d(audio_channels, inner_channels,
    ...
    def forward(self, data):
        ...
        """
        audio, cond, diffusion_steps = data

        # Get fourier embedding for timestep t
        diffusion_step_embed = calc_diffusion_step_embedding(diffusion_steps, self.diffusion_step_embed_dim_in)

        # Run fourier features through linear layers for learned embedding
        diffusion_step_embed = swish(self.fc_t1(diffusion_step_embed))
        diffusion_step_embed = swish(self.fc_t2(diffusion_step_embed))

        # Run the first convolution to get to a fixed number of channels (inner_channels)
        ...
        for n, audio_down in enumerate(reversed(downsample)):
            x = self.lvc_blocks[n]((x, audio_down, cond, diffusion_step_embed))
